...
class SshKeyPairSecretArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] secret_name: Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and
               underscores and must begin with a letter or digit.
        :param pulumi.Input[bool] clean_ssh_key: Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only
        ...
        :param pulumi.Input[str] kms_key_id: Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default
               will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        :param pulumi.Input[str] ssh_key_name: Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The
...
class SshKeyPairSecret(pulumi.CustomResource):
    ...
    def __init__(__self__,
                 ...
                 __props__=None):
        """
        Create a SshKeyPairSecret resource with the given unique name, props, and options.
