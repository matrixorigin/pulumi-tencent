...
class AccelerationDomainArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[int] http_origin_port: HTTP back-to-origin port, the value is 1-65535, effective when OriginProtocol=FOLLOW/HTTP, if not filled in, the default
               value is 80.
        :param pulumi.Input[int] https_origin_port: HTTPS back-to-origin port. The value range is 1-65535. It takes effect when OriginProtocol=FOLLOW/HTTPS. If it is not
               filled in, the default value is 443.
        :param pulumi.Input[str] ipv6_status: IPv6 status, the value is: `follow`: follow the site IPv6 configuration; `on`: on; `off`: off. If not filled in, the
               default is: `follow`.
        :param pulumi.Input[str] origin_protocol: Origin return protocol, possible values are: `FOLLOW`: protocol follow; `HTTP`: HTTP protocol back to source; `HTTPS`:
...
class AccelerationDomain(pulumi.CustomResource):
    ...
    def __init__(__self__,
                 ...
                 __props__=None):
        """
        Create a AccelerationDomain resource with the given unique name, props, and options.
